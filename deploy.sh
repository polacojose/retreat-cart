...
build() {
	...
	if test -f Dockerfile; then
		...
		tar -cf - * | gzip -9 | kubectl run ${jobName} --stdin=true --image=gcr.io/kaniko-project/executor:debug -n gocd --restart=Never \
		  -- --dockerfile=Dockerfile --context=tar://stdin --destination=registry.polacoproject.net/retreat-cart:${VERSION} --destination=registry.polacoproject.net/retreat-cart:latest --cache=true
  ...
  echo "Finished building retreat-cart:${VERSION}"
}
...
update_deployment() {
	...
	kustomize edit set image registry.polacoproject.net/retreat-cart:${VERSION}
	cd -
